...
public class GlobalApi {
    ...
    public void carLogin(String phoneNum,String password){
        ...
        if(PublicUtil.properties.getProperty("EnvPropName").equalsIgnoreCase("EnvTrainConfig")){
            headMap.put("currentev","train");
        }
...
//        获取maxID表中的t_laonApply , t_AmountAdvances的Maxid
    ...
    public void insertViewRecord(String phone){
        ...
        String sql_insertCar = "INSERT INTO t_CarHistory (FCustID,FCarID,FHistoryDate,FIsDelete)\n" +
        ...
        String sql_insertEntry = "INSERT INTO t_CarHistoryEntry (FCarHistoryID,FAddDate)\n" +
     ...
     * @param mobile
     */
    public void inquiryApply(String mobile){
        String sqlToken ="select top 1 accessToken from t_AuthAccessToken where account = '"+unBankUser+"' order by id desc";
        String sqlCarID = "select top 1 FCarID from t_CarSalesInfo a INNER JOIN t_CarInfoNew b ON a.FCarID=b.FID " +
                "INNER JOIN t_Member c ON b.FCustID=c.FMemberID where a.FSaleStatus=1 and a.FIsDelete=0 and " +
                "c.FMobile='"+mobile+"';";
        String token =sqlUtil.getColumnValue(sqlToken,"accesstoken");
        String carID =sqlUtil.getColumnValue(sqlCarID,"FCarID");
        //如果该用户已经询价过该车辆就先删除以前的询价记录
        deleteInquiryRecord(unBankUser,carID);

        LinkedHashMap<String,Object> bodyMap = new LinkedHashMap<String, Object>();
        LinkedHashMap<String,Object> headMap = new LinkedHashMap<String, Object>();
        headMap.put("AccessToken",token);
        headMap.put("version","2.5.0");
        bodyMap.put("carIdArray",carID);
        bodyMap.put("inquiryPrice","10000");
        bodyMap.put("mobile",unBankUser);
        String url = subCarUrl + "/m/sell/inquiry/apply";
        try {
            JSONObject resultJson = httpUtil.postForm(url, headMap, bodyMap);
    ...
    /**
     * 删除这个手机号码的询价这辆车的记录，把delete设置为1
     * @param mobile 询价人的手机号
     * @param carID 被询价的车辆ID
     */
    public void deleteInquiryRecord(String mobile,String carID){
        String sql_1 = "select FID from t_Car_Inquiry_Record a INNER JOIN t_Member b ON a.FInquirerID " +
                "= b.FMemberID where a.FCarID="+carID+" and FisDelete=0 and b.FMobile='"+mobile+"';";
        String FID = sqlUtil.getColumnValue(sql_1,"FID");
        if(FID==null){
            return;
        }
        String sql_2 ="update t_Car_Inquiry_Record SET FisDelete=1 where FID="+FID+";";
        String sql_3 = "update t_Car_Inquiry_Record_Entry SET FIsDelete=1 where FInquiryRecordID="+FID+";";
        //删除t_Car_Inquiry_Record的记录
        sqlUtil.updateColumnValue(sql_2);
        //t_Car_Inquiry_Record详细记录
        sqlUtil.updateColumnValue(sql_3);
        log.info("删除询价记录成功");
    }

    public static void main(String[] args){
        GlobalApi  globalApi = new GlobalApi();
        globalApi.scbAmountReturnPass("17455413214");
//        globalApi.inquiryApply("17455413214");
...
//        globalApi.ysbAmountReturnPass("17455413214");
    }
